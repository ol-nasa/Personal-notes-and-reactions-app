# Personal Notes & Reactions App 📝

Веб-приложение для ведения заметок с иерархической системой реакций. Записи синхронизируются между устройствами через облако.

## ✨ Особенности

- 📌 **Факты и реакции** - организуйте мысли иерархически
- 🎨 **Динамическая раскраска** - записи окрашиваются по времени создания
- ☁️ **Облачная синхронизация** - Firebase синхронизирует данные между всеми устройствами
- 📱 **Адаптивный дизайн** - работает на телефоне, планшете и ПК
- 💾 **Локальное хранилище** - резервное копирование в localStorage
- 📊 **CSV экспорт/импорт** - скачивайте и переносите записи
- 🌙 **Тёмный режим** - следует предпочтениям системы
...
```bash
python3 -m http.server 8000 --bind 0.0.0.0
# На другом устройстве откройте http://<ваш_ip>:8000
```

### С облачной синхронизацией (Firebase)

1. **[Настройте Firebase](./FIREBASE_SETUP.md)** - потребуется бесплатный аккаунт
2. **Вставьте конфиг** в `index.html`
3. Готово! Данные синхронизируются автоматически

## 📖 Как пользоваться

### Добавление записи (факт)
1. Напишите текст в поле
2. Нажмите **"Записать"** (или Enter)
3. Запись появится в ленте

### Ответ на запись (реакция)
1. Нажмите **"Ответить"** на нужной записи
2. Напишите текст
3. Нажмите **"Ответить"** - появится как вложенная запись

### Управление записями
- **Свернуть/развернуть** - нажмите треугольник ▾/▸
- **Удалить** - нажмите "Удалить" → "Точно?" (подтверждение)
- **Экспорт** - нажмите "Экспорт CSV" → "Скачать CSV"
- **Импорт** - нажмите "Экспорт CSV" → "Загрузить CSV"

## 🎨 Раскраска по времени

Записи окрашиваются автоматически в зависимости от времени создания:
- 🍑 06:00-11:00 - персиковый (утро)
- 💛 11:00-14:00 - жёлтый (день)
- 💚 14:00-17:00 - зелёный (полдень)
- 💙 17:00-21:00 - голубой (вечер)
- 🔵 21:00-23:00 - синий (ночь)
- 💜 23:00-06:00 - фиолетовый (поздняя ночь)
...
- Каждый браузер имеет свой набор данных
- Данные сохраняются даже после закрытия приложения

### С Firebase
- Данные синхронизируются на облако
- Доступны на всех устройствах
- Видимы в реальном времени (~2 сек задержка)

## 📊 Экспорт данных

Любые записи можно экспортировать в CSV:
- **Формат**: ДД.ММ.ГГ ч:мм:сс
- **Кодировка**: UTF-8 с BOM (совместимо с Excel)
- **Разделитель**: точка с запятой (;)

```csv
id;parent_id;content;created_at
uuid-123;;Первая запись;26.07.26 14:30:45
uuid-456;uuid-123;Комментарий;26.07.26 14:31:12
```

## 🔐 Конфиденциальность

- **Без Firebase**: все данные остаются на вашем устройстве
- **С Firebase**: данные зашифрованы в пути и в базе данных, доступ только вам

## 🌐 Развёртывание

### GitHub Pages (бесплатно)
1. Зайдите в Settings репозитория
2. Pages → Deploy from a branch → main
3. Приложение будет доступно по URL: `https://username.github.io/Personal-notes-and-reactions-app/`

### Другой хостинг
- Netlify, Vercel, Firebase Hosting - просто подключите репозиторий

## 💡 Советы

- **CSV для синхронизации между браузерами**: экспортируйте на одном, импортируйте на другом
- **Firebase для автоматической синхронизации**: настройте один раз, забудьте
- **Тёмный режим**: следует системным предпочтениям (Параметры → Дисплей)

## 🛠️ Разработка

Приложение написано на чистом JavaScript (без фреймворков):
- Ванильный JS - 0 зависимостей
- Firebase SDK - для облачной синхронизации (опционально)
...
**Вопросы?** Откройте консоль браузера (F12) для отладочных сообщений.
